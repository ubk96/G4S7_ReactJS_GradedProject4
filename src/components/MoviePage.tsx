...
const MoviePage = () =>
{
  ...
  const [movieType,setMovieType]  = useState<'movies-in-theaters' | 'movies-coming' | 'top-rated-india' | 'top-rated-movies' | 'favourites'>('movies-in-theaters');
  const [favouriteTab, setFavouriteTab] = useState<boolean>(false);
  const newTabClick = (newTab : 'movies-in-theaters' | 'movies-coming' | 'top-rated-india' | 'top-rated-movies' | 'favourites' ) =>
  {    
